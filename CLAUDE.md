...
The five backend applications are **Spring Boot 4.0.7 on Java 25**, generated by **JHipster 9.2.0**.

**`web/` is the sixth application and it is not one of the five** (D101, backlog NEW-48 Stage A). A
**client-only** JHipster 9.2.0 / Angular 21.2.17 app — `skipServer: true`, `databaseType: no`, so no
`pom.xml`, no `mvnw` and **zero `.java` files** — generated into its own directory precisely so the
gateway is never regenerated: `--force` there hands back D61's working `admin`/`admin` credential and
rewrites the OpenTelemetry block out of `pom.xml`. `enableTranslation` is **on** and `jhiPrefix` is
...
# Regenerate an app from its JDL — see "the regeneration hazard" before doing this, and see
# "Generating inside an agent worktree" before running ANY jhipster command from .claude/worktrees/
cd catalog && jhipster jdl ../jdl/catalog.jdl --skip-install --skip-git --force

# The marketplace client (D101). Node 24.18.0 — `nvm use` reads web/.nvmrc. No mvnw, no Java.
cd web && npm ci
cd web && npm run lint && npm test          # `pretest` runs lint, so `npm test` covers both
cd web && npm run prettier:check            # lint does NOT cover .scss/.md/.json — see the CI section
cd web && npm run webapp:prod && npm run check:built-assets   # build, THEN assert over the output
cd web && HC_GATEWAY_PORT=15509 npm start   # ⚠ the default 8080 is abofonsa_api on this box
...
### ⚠ Generating inside an agent worktree produces a broken app, and the generator exits 0

**This bites `web/` and would bite any JHipster generation run from `<repo>/.claude/worktrees/agent-*`,
which is where every agent in this workspace works.** Found and measured on 2026-09-24 (D101 §2).

`generators/angular/support/translate-angular.js` decides which files the i18n transform touches with

```js
const minimatch = new Minimatch('**/*{.html,.ts}');          // note: no `dot: true`
export const isTranslatedAngularFile = file => minimatch.match(file.path);
```

matched against the file's **absolute** path. Minimatch's `**` will not cross a path segment that
begins with a dot, so under `.claude/` it matches **nothing**. The transform is still queued, its name
still prints — *"- translating angular application"* — it processes zero files, and the generator
reports success:

```
✔ Angular 21.2.17 application generated successfully.     exit 0, 241 files created
19 files left holding raw  __jhiTranslateTag__('home.title')
2466 eslint errors, every one prettier/prettier
```

Raw `__jhiTranslateTag__(…)` is not valid Angular, so five of those files do not parse — this is loud
eventually and silent at the moment it happens. **Re-running `--force` over the wreckage misleads**: it
now fails with `ERROR! Error parsing file src/main/webapp/app/home/home.html`, because the prettier
transform meets the placeholders the *first* run left, and a reader who starts there debugs prettier.

**The trigger is a dot-segment anywhere in the absolute path — not a worktree, and not git.**
`~/.cache/…` reproduces it. **The remedy is to generate somewhere with no dot-segment and copy the tree
in**; `package.json` comes out byte-identical, so an existing `node_modules` and lockfile survive. Check
it with `grep -rl '__jhiTranslate' src | wc -l`, which must be `0`, and `eslint . --fix`, which must
change nothing.

The config is **not** the variable, and checking it first wastes the afternoon: two scratch generations
from the same `.yo-rc.json` — differing only in `creationTimestamp` — produced an identical 241-file
list with **0** placeholders. Identical inputs, identical outputs, different results; what differed was
*where*. This is the workspace guide's *"a tool that walks the repository from its root now has a second
application inside it"* family **inverted** — here the tool's own glob refuses the directory the work is
being done in.
...
- **`web`** — the marketplace client (D101). Node, not Java, so its own job rather than a sixth matrix
  entry: `npm ci`, `lint`, **`prettier:check`**, `test`, `webapp:prod`, then **`check:built-assets`**.
  **`prettier:check` is not redundant beside `lint`** — `eslint-plugin-prettier` enforces formatting
  only for files ESLint has a configuration for, and `npx eslint` on a `.scss` or a `.md` answers *"File
  ignored because no matching configuration was supplied"* and exits **0** (measured). So `.scss`,
  `.md` and `.json` were ungated, `_hc-tokens.scss` — which `brand-contrast.spec.ts` parses off disk —
  among them. The last step is the
  point and it is last for a reason — the first steps read **source**, while the Sass-publishing defect
  D90 §3 names is a property of what `ng build` **copies**, and `angular.json`'s glob is exactly the
  thing that looks right while the directory ships anyway. `hc-admin/app` publishes its own design
  tokens on `admin.abofonsa.com` today because nothing there ever looked at built output. Neither rule
  in that check reads `angular.json`; one is a literal pattern over the output tree, the other derives
  what *should* have been copied from the **source** directory, so a glob narrowed too far (`["**"]`
  publishes no Sass and quietly copies no images) fails as well as one left too wide.
  `CYPRESS_INSTALL_BINARY=0` — **nothing runs Cypress anywhere**, because the suite needs a running
  gateway and an origin to serve the client from, and neither exists; drop that variable in the change
  that first runs e2e.
