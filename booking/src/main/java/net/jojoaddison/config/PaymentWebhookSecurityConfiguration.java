 ...
 * Nothing routes here from outside today — the gateway's four route predicates match
 * {@code /services/<service>/api/**}, and this path is not under {@code /api}, which is the same
 * property that keeps catalog's {@code /internal/**} private (D28). And everything that is not a POST
