 ...
 *
 * <p>{@code Booking.scheduledTime} was a {@code String maxlength(5)} until decisions.md D26.
...
public final class SlotTime {
    ...
    public static LocalTime parse(String value) {
        if (value == null || value.isBlank()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "a scheduled time is required, as HH:mm");
        }
        try {
            return LocalTime.parse(value.trim(), HH_MM);
        } catch (DateTimeParseException notATime) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "'%s' is not a valid scheduled time — expected HH:mm, e.g. 07:00".formatted(value));
        }
    }
}
